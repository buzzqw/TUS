...
Chi ha l'iniziativa tra giocatori e nemici più alta incomincia per primo e successivamente agiscono gli altri in ordine decrescente, dichiarando le Azioni ed eseguendole. In caso di Iniziativa di pari punteggio agisce per primo chi ha il punteggio della Caratteristica più alto, altrimenti lo scontro sarà in contemporanea. L'iniziativa vale per l'intero scontro e si ritira al cambio dell'avversario.\index{Iniziativa uguale}

\begin{narratore}[Gestire il combattimento] %box narratore
Cercate di far fluire il combattimento in maniera naturale. Non interrompete il flusso delle azioni, ma descrivetene gli effetti e coinvolgete i giocatori (e i nemici) nelle azioni seguenti. Vi consiglio la lettura dell'articolo \href{https://theangrygm.com/manage-combat-like-a-dolphin/}{How to Manage Combat Like a Dolphin} per capire nel dettaglio il metodo.
\end{narratore}
...
Un \textbf{round} dura circa 10 secondi, un lasso di tempo sufficiente per agire, correre, parlare e combattere. Un minuto è quindi composto da 6 round e un turno dura 10 minuti (o 60 round).\index{Round e Turno, durata}
...
\subsubsection{Tempo di riattivazione Oggetti ed Abilità}\index{Tempo di attivazione Oggetti ed Abilità}\label{temporiattivazioneoggetti}\index{Ricarica oggetti magici}
...
Se l'iniziativa tirata è la più veloce ed è di +8 o più punti superiore alla seconda allora in quel round potrà usare un'Azione di Reazione od Immediata in più, se il differenziale è di almeno +16 la sua grande reattività gli permette di eseguire un'Azione in più.\index{Critico nell'Iniziativa}
...
\textbf{Un'Azione non può essere interrotta da un'altra Azione, ma può essere seguita da un'Azione di Reazione o da un'Azione Immediata}. \index{Interrompere Azioni}\index{Azioni, Interrompere}
...
Le \textbf{Azioni Gratuite} possono essere usate in qualsiasi momento.
...
Il personaggio che esegue un'Azione di Attacco ed il Lancio di un Incantesimo nel medesimo round si considera Distratto ovvero deve eseguire una Prova di Magia per lanciare l'incantesimo.\index{Attacco ed Incantesimo}

\textbf{Azione di Movimento*}: un'Azione di Movimento è un'Azione dedicata a spostarsi. Ci si può spostare fino a tutto il proprio movimento (9 metri per umani, 6 metri per nani...) per ogni Azione usata. Ogni movimento consuma un'Azione, anche se non si sfrutta tutto il proprio movimento a disposizione.\index{Azione di Movimento}

Durante l'Azione di Movimento è possibile \textbf{Estrarre l'Arma} o lo Scudo, oppure \textbf{Rinfoderare l'Arma} o lo Scudo.
...
\midrule
\hyperlink{tipodimovimento}{Eseguire un'Azione di Movimento} &1*\\
...
\hyperlink{spingereavversario}{Spingere un avversario} & 1\\
\rowcolor{gray!20}\hyperlink{afferrareunavversario}{Afferrare l'avversario} & 2\\
\hyperlink{farecadereavversario}{Fare cadere l'avversario} & 1
\end{tabularx}
...
\textbf{Scambiare poche battute con qualcuno*}: Finché si tratta veramente di poche battute o di uno sguardo, non si consumano Azioni; se il dialogo diventa più articolato, allora utilizza delle Azioni. L'obiettivo è non interrompere il flusso delle Azioni con un fitto dialogo, ma comunque permettere l'interazione tra i personaggi.

\textbf{Eseguire prova su una competenza*}: se sfrutta una frazione del round, costa 1 Azione, altrimenti 2 o più. Controllate negli \hyperlink{esempiprovecompetenze}{Esempi Prove Competenza} i costi riportati.

Un'Azione di \textbf{Reazione (R)} \index{Azione di Reazione} può essere eseguita liberamente anche fuori dal proprio round. Questa Azione è solitamente dovuta ad Abilità o situazioni particolari. Se non indicato diversamente, un'Azione di Reazione accade immediatamente dopo la causa che la scatena.

Un'Azione \textbf{Immediata (Imm.)} \index{Azione Immediata} può essere eseguita liberamente nel proprio round, prima o dopo la propria Azione. Un'Azione Immediata è solitamente concessa da particolari Abilità.
...
L'\textbf{ordine} con cui si eseguono le Azioni non è importante se non per la correlazione logica e fisica. L'Azione di Movimento può essere tra altre Azioni (movimento, attacco/incantesimi/altra Azione, movimento).
...
Se si esegue un'Azione di \textbf{Scatto} \index{Scatto}si raddoppiano i metri percorsi (2x9 metri per un umano), per un nano (Movimento 6m) significa fare 12 metri, in un'Azione.
È anche possibile fare più Azioni di Scatto, fino a 3 in un round, ovvero correre per 6 volte il proprio movimento.

Il personaggio che fa un'Azione di Scatto \index{Azione di Scatto} corre e ha una penalità di 1d6 nel Tiro per Colpire; la Difesa diminuisce di 4 fino all'inizio del suo round successivo e si considera Distratto per il lancio di incantesimi.\index{Penalita' per correre}
...
Quando si parla di \textbf{\emph{quadretto}} \index{Quadretto} per indicare una distanza o un'influenza si intende un quadretto di mappa di 1 metro x 1 metro.
...
Nel Mostruario sono indicate le dimensioni e i relativi spazi occupati dalle creature di \hyperlink{tagliaedimensioni}{taglia} diversa (pag. \pageref{tagliaedimensioni}).

\subsection{Distanza}\index{Distanza}\label{distanza}

Per \textbf{distanza di Tocco} \index{Distanza di Tocco} \index{Tocco} si intende una distanza che permette di toccare l'avversario, quindi non più di un metro per creature di taglia media senza armi lunghe o con portata. La distanza di tocco è una distanza di mischia qualora non si usino armi lunghe.

Per \textbf{distanza di Mischia} \index{Distanza di Mischia} \index{Mischia} si intende una distanza che permette il combattimento corpo a corpo, determinata dalla lunghezza della propria arma. L'area che può essere colpita attorno al personaggio viene chiamata anche \textbf{area di minaccia}\index{Area di minaccia} (solitamente entro 1 metro attorno al personaggio, oppure entro 2 metri in caso di arma lunga).

Nei mostri questa distanza è indicata dalla portata, per le armi da lancio è chiamata gittata.

Se non indicata nella scheda dell'avversario, la \textbf{portata} è pari a metà dello spazio occupato, arrotondata per eccesso. Un gigante delle colline, di taglia Enorme (3x3 quadretti sulla mappa), ha portata 2 quadretti, ovvero colpisce creature entro 2 quadretti/metri da lui.\index{Taglia e distanza di mischia}\index{Portata}
...
Il danno da arma si calcola come somma del dado dell'arma, della Forza (o della Destrezza se indicato da Abilità), sia essa positiva o negativa, dei bonus dati da Lista d'Armi, dei bonus dati dalle Abilità, dei bonus dati dall'arma e dei bonus circostanziali.\index{Come calcolare il danno dell'arma}\index{Danno dell'arma}

Quando una creatura raggiunge 0 (zero) Punti Ferita si considera svenuta\index{Svenuto}, ovvero Indifesa e Inabile a fare qualsiasi cosa. Una Cura magica (Incantesimo, Pozione...) la porterà cosciente e ai Punti Ferita curati. Una prova di \hyperlink{prontosoccorso}{Pronto Soccorso} (pag. \pageref{prontosoccorso}) (DC 12) potrà essere usata per riportarla cosciente a 1 Punto Ferita.
Se lasciata svenuta per un'ora, se non è successo qualcosa a mutare la situazione, la creatura può fare un Tiro Salvezza su Tempra a DC 15; se riesce torna a 1 Punto Ferita, se fallisce va a -1 e diventa morente.\index{Zero PF, recupero da}\index{0 Punti Ferita}

Un personaggio morente ha Punti Ferita negativi (-1 o meno), è svenuto ed è \hyperlink{morente}{indifeso}. Continuerà a perdere 1 Punto Ferita a round finché il valore non raggiungerà il doppio della Costituzione +10 e il personaggio morirà, se non viene curato.
...
Es. Tups è gravemente ferito e ha attualmente -6 Punti Ferita, Jade decide di provare a curarlo (dopo averlo spostato in un posto più sicuro). Jade tenta una prova di Pronto Soccorso (3 Azioni) per stabilizzare il compagno; la sua difficoltà alla prova è 12+6, ovvero deve superare con Pronto Soccorso DC 18 per riportarlo a 0 Punti Ferita (svenuto).
...
Le \textbf{Condizioni} \index{Condizioni mentali}\textbf{di tipo mentale}, quali Affascinato e Confuso ma non Dominato, terminano quando il personaggio diventa morente.
...
Quando un personaggio arriva a Punti Ferita negativi pari a 10+doppio del suo punteggio di Costituzione è \hyperlink{morto}{\textbf{morto}} [-(10+(COS*2))].
...
Se il danno non letale di un personaggio arriva a Punti Ferita negativi pari a 20+4*Costituzione, il personaggio è morto.\hypertarget{puntiferitatemporaneimorte}{}
...
Chi si \textbf{difende} ha una \textbf{Difesa} pari a: 10 + Destrezza + Scudo + Armatura + bonus magici + Abilità e bonus circostanziali (copertura, ad esempio). Per i mostri il valore della Difesa è già calcolato sulla base dei valori normali e sarà eventualmente da modificare in base alla situazione.

Per Difesa naturale si intende il punteggio di 10 alla base del calcolo della Difesa; alcune eccezionali Abilità possono aumentare questo valore.\index{Difesa naturale}
...
Se il \textbf{Tiro per Colpire} è pari o superiore al valore della Difesa, l'avversario è stato colpito e si stabilirà il danno della ferita, dato dal dado dell'arma + punteggio di Forza e altri fattori quali bonus magici, Lista d'Armi e Abilità.
...
Se è \textbf{sufficiente toccare l'avversario}, il Tiro per Colpire ha +1d6 di bonus, dato che non è necessario portare il colpo, ma solo sfiorarlo.\index{Toccare l'avversario}. Nel manuale è chiamato Attacco a Tocco.\index{Attacco a tocco}\label{attaccoatocco}\hypertarget{attaccoatocco}{}\label{difesaatocco}

Se \textbf{l'avversario è sorpreso}, ovvero non si aspetta l'attacco, la Difesa e il Tiro Salvezza su Riflessi avranno una penalità di -2. Questo è il valore della \textbf{Difesa di sorpresa}.\label{difesasorpresi}
...
OBSS vuole essere divertente da giocare, vuole che i giocatori si divertano e vedano i risultati ottenuti dai dadi (e ovviamente dalle loro scelte). Le Golden Rules e l'Esplosione del Danno vogliono proprio togliere la patina di polvere ai dadi e far divertire. Un giocatore apprezzerà, ancor più se esperto, il fatto che i tiri dei dadi non siano solo un numero, ma aprano la possibilità di fare la differenza. Chiedete al giocatore di descrivere il colpo critico e fatelo recitare nella sua gloria di potenza!
\end{narratore}
...
Se hai tirato tre volte 1 hai mancato, indipendentemente dal risultato finale. Il Narratore potrebbe anche decidere che succedano brutte cose... (ad esempio vedi \hyperlink{tabellafallimentiarmi}{Tabella Fallimento Tiri per Colpire}, pag. \pageref{tabellafallimentiarmi})
...
Ogniqualvolta hai colpito, tiri un \textbf{danno aggiuntivo della sola arma} per ogni margine di 8 con cui hai superato la Difesa con l'intero Tiro per Colpire; questo danno viene anche chiamato \textbf{danno critico}. Se hai fatto due Tiri Critici vuol dire che devi tirare 2 dadi dell'arma in più e che hai colpito con un margine tra +16 e +23!

\begin{giocatore}[Esempio Tiro Critico]
Es. tiro 6 4 5, tiro in aggiunta 6, tiro in aggiunta un 6, tiro in aggiunta 4, totale 31. La Difesa dell'avversario è 15. Come danno tiri 3 volte il danno dell'arma, una volta perché ho colpito e due perché l'ho colpito con un margine di 16!
\end{giocatore}
...
Con \textbf{un'Azione} il personaggio può eseguire un \textbf{singolo Tiro per Colpire}.
...
Se l'obiettivo è entro la gittata indicata non si hanno penalità al colpire; se il bersaglio è tra il primo e il secondo incremento, la penalità al colpire è -6. Se il bersaglio è tra il secondo e il terzo incremento, la penalità al colpire è di -12.\index{Penalita' distanza}
...
\begin{giocatore}[Combattimento con Arma Lunga]
		Es. Tups armato di spada lunga affronta un brigante armato di lancia lunga. Tups ha iniziativa 15, il brigante 12.
...
Queste armi sono leggere e indicate per il \hyperlink{combattimentoaduemani}{combattimento a due armi}.
...
L'avversario deve essere entro 2 Azioni di movimento (18 o 12 metri solitamente) e a non meno di 3 metri; il terreno non deve essere difficile (vedi anche Abilità \hyperlink{Rinoceronte}{Rinoceronte}, pag. \pageref{Rinoceronte}). Si deve correre fino a essere a distanza di mischia.
...
\textit{Nota: La progressione degli attacchi durante la carica (+1d6, -10, -15, -20...) è diversa dagli attacchi multipli standard (0, -5, -10, -15...). Questa differenza è intenzionale: effettuare un secondo attacco dopo una carica è più difficile rispetto ad attacchi multipli normali.}
...
Sono armi a spargimento quelle che \emph{spargono} il loro contenuto dove cadono, ad esempio olio incendiato/Acqua santa... Un'arma a spargimento ha una gittata di 6 metri\index{Lanciare Armi a Spargimento}\index{Gittata armi a spargimento}.
...
\textbf{X} indica il bersaglio dell'oggetto tirato. \textbf{0} indica il punto di origine del lancio.
...
%\textbf{+1}& & \\
%
\rowcolor{gray!20}\textbf{+2}& Fiancheggia, Posizione Sopraelevata, Attacca alle spalle& Copertura leggera\\
...
\begin{narratore}[Non sono tutte regole]
...
Se non si è presa la Lista d'Armi \emph{Pugno Vuoto}, un pugno o un calcio infligge 1d3 + Forza di danno non letale. Solo con la Lista d'Armi Pugno Vuoto si diventa artisti marziali.
...
Quando sei prono puoi strisciare\index{Strisciare}\index{Carponi} o muoverti a carponi. Il terreno si considera difficile e sei comunque considerato prono finché non ti alzi.
...
Oltre alle eventuali penalità date dalla \hyperlink{esempicopertura}{Copertura} (pag. \pageref{esempicopertura}), si ha una penalità aggiuntiva di -2 al Tiro per Colpire.
...
Nella tabella delle armi la dimensione è segnata come P (piccola), M (media), G (grande), E (enorme) ed è riferita a una creatura di taglia media. Una versione \emph{più grande} di un'arma aumenta di una categoria il danno dell'arma (1d4->1d6, 1d6->1d8, 1d8->1d10, 1d10/1d12->2d6, 2d6->2d8, 2d8->2d10, 2d10->3d6...).
...
Queste Azioni di combattimento sono a discrezione del Narratore, che può concederle o meno. \textbf{Ogni manovra conta come Azione di Attacco} per quanto riguarda le penalità del multiattacco.\index{Manovre ed Azioni di Attacco}
...
È una prova di Atletica contrapposta a un Tiro Salvezza su Tempra con Forza. Chi ha una taglia maggiore guadagna un bonus di +1d6 per ogni taglia di differenza. Se chi riesce nella manovra ottiene un Successo Critico, si considera che abbia \hyperlink{bloccato}{Bloccato} l'avversario.
...
\index{Attraversare nemici}\index{Movimento attraverso}Un personaggio può \textbf{attraversare}, ma non sostare in \textbf{una zona occupata} da una creatura senza essere \hyperlink{ristretto}{\textbf{ristretto}}\index{Ristretto}.

Per attraversare il terreno dove c'è una creatura ostile, o che è in portata di attacco della creatura ostile a cui si passa a fianco, è necessario eseguire una Prova Contrapposta di Atletica o Acrobatica contro il Tiro Salvezza su Riflessi della creatura il cui terreno si vuole \textbf{attraversare}; ogni creatura attraversata oltre la prima cumula una difficoltà di +2.
...
Se si fallisce, si rimane nel quadretto immediatamente precedente al nemico, con il rischio di essere \hyperlink{ristretti}{ristretti} (pag. \pageref{ristretti}). Si considerano terminate sia l'Azione di Movimento sia quella di oltrepassare.
...
Ogni round può \emph{usare} uno o più Tiri Critici accumulati per eseguire Azioni Critiche. L'uso delle Azioni Critiche deve essere contro l'avversario al quale sono stati effettuati i Tiri Critici.
...
\item \textbf{3}: Bersaglio abbagliato. Tira 1d6: con 1-2-3 l'avversario manca il suo attacco. Dura fino alla fine del prossimo round.
...
\item \textbf{1}: Ti puoi spostare di un'Azione di Movimento come Reazione. Tratti il terreno come difficile.

\item \textbf{2}: Come al punto 1, ma la distanza iniziale è di 6 metri.

\item \textbf{2}: Ti puoi spostare di un'Azione di Movimento come Reazione.

\item \textbf{3}: Come al punto 1, ma la distanza iniziale è di 9 metri.

\item \textbf{4}: Come al punto 3, e puoi spostarti con l'avversario.
	\end{itemize}
	...
	\begin{itemize}[leftmargin=*]
		\setlength{\itemsep}{0pt}
\item \textbf{1}: Spallata. L'avversario esegue un Tiro Salvezza su Tempra DC 15, $ \pm 4 $ per differenza di taglia, oppure cade prono.

\item \textbf{2}: Sgambetto. L'avversario esegue un Tiro Salvezza su Riflessi DC 19, $ \pm 2 $ per differenza di taglia, oppure cade prono.

\item \textbf{3}: Spinta. L'avversario esegue un Tiro Salvezza su Tempra DC 23, $ \pm 2 $ per differenza di taglia, oppure cade prono. Se il Tiro Salvezza riesce, viene allontanato di 1d6 metri.

\item \textbf{4}: Urto. L'avversario esegue un Tiro Salvezza su Tempra con DC pari al tuo ultimo Tiro per Colpire, $ \pm 2 $ per differenza di taglia, oppure cade prono. Se il Tiro Salvezza riesce, viene allontanato di 1d10 metri.
	\end{itemize}
	...
	\begin{itemize}[leftmargin=*]
		\setlength{\itemsep}{0pt}
\item \textbf{Vari}: Per ogni Tiro Critico usato, aggiungi un'ulteriore gittata alla tua arma.

\item \textbf{Vari}: Per ogni due Tiri Critici usati, aggiungi +4 al prossimo Tiro per Colpire entro la fine del round successivo.
	...
	\begin{itemize}[leftmargin=*]
		...
		\item \textbf{2}: Berserker. Un tuo compagno entro sei metri può eseguire un'Azione di Attacco contro un avversario in mischia al costo di una Reazione.

		\item \textbf{3}: Scossa. Un tuo compagno, entro 6 metri, può usare una Reazione per eseguire un'Azione di Movimento trattando il terreno come difficile
...
\begin{giocatore}[Ho mancato!]
		Sfruttate al meglio la vostra sfortuna! Fate che la sorte vi possa sorridere grazie alla vostra abilità nello scegliere la manovra da attivare. Valutate sempre l'ambiente, i nemici, i compagni e la situazione in genere prima di decidere cosa attivare.
\end{giocatore}
...
\begin{itemize}[leftmargin=*]
	...
	\begin{itemize}[leftmargin=*]
		...
		\item \textbf{Finta e Riposizionamento} - Attiv.: \textbf{\emph{Mancato}} con un \textbf{\emph{pari}}. \emph{Effetto}: Muovi 1 metro senza provocare attacchi di opportunità. \emph{Critico}: Come sopra, ma ti muovi di 2 metri.

		\item \textbf{Colpo Distraente} - Attiv.: \textbf{\emph{Mancato}}. \emph{Effetto}: +1 alla Difesa contro l'avversario fino alla fine del prossimo round. \emph{Critico}: L'avversario considera il personaggio come se avesse copertura leggera.
	...
	\begin{itemize}[leftmargin=*]
		\setlength{\itemsep}{0pt}
		\item \textbf{Reazione Rapida} - Attiv.: \textbf{\emph{Mancato}}. \emph{Effetto}: il personaggio può bere una pozione. \emph{Critico}: Il personaggio può somministrare una pozione a un alleato adiacente.
		...
		\item \textbf{Contrattacco Immediato} - Attiv.: \textbf{\emph{Dispari}}. \emph{Effetto}: Se il Tiro per Colpire fallito avrebbe colpito un avversario adiacente, infliggi danni pari al modificatore di Forza. \emph{Critico}: Infliggi danni pari al doppio del modificatore di Forza.
	...
	\begin{itemize}[leftmargin=*]
		...
		\item \textbf{Attacco a Sorpresa} - Attiv.: \textbf{\emph{Pari}}. \emph{Effetto}: Usando una Reazione puoi effettuare un Attacco con il medesimo Tiro per Colpire dell'ultimo attacco. \emph{Critico}: Se il tuo Tiro per Colpire è sufficiente, colpisce un altro avversario in mischia.
	\end{itemize}
	...
	\begin{itemize}[leftmargin=*]
		\setlength{\itemsep}{0pt}
		\item \textbf{Manovra Laterale} - Attiv.: \textbf{\emph{Mancato}}. \emph{Effetto}: Ti muovi di 1 metro. \emph{Critico}: Ti muovi fino a 4 metri, ma il prossimo round esegui un'Azione in meno.
		...
		\item \textbf{Intimidazione Superiore} - Attiv.: \textbf{\emph{Dispari}}. \emph{Effetto}: L'avversario subisce -4 al primo attacco contro di te entro la fine del prossimo round. \emph{Critico}: Entro la fine del prossimo round, l'avversario in mischia non può infliggere danni critici contro di te.
		...
		\item \textbf{Valutazione Strategica} - Attiv.: \textbf{\emph{Pari}}. \emph{Effetto}: Il prossimo attacco a segno entro la fine del prossimo round infligge un danno critico in più. \emph{Critico}: Come sopra, ma infligge due danni critici; il round successivo esegui un'Azione in meno.
	\end{itemize}
	...
	\begin{itemize}[leftmargin=*]
		\setlength{\itemsep}{0pt}
		\item \textbf{Assalto Incessante} - Attiv.: \textbf{\emph{Mancato}}. \emph{Effetto}: Il prossimo round hai un +1 cumulativo al Tiro per Colpire per ogni volta che attacchi. \emph{Critico}: Il prossimo round hai solo 1 Azione. Se la usi per attaccare e colpisci, causi 2 danni critici in più.
		...
		\item \textbf{Intuizione Letale} - Attiv.: \textbf{\emph{Pari}}. \emph{Effetto}: Il prossimo attacco andato a segno entro la fine del round successivo infligge danni critici massimizzati. \emph{Critico}: Come sopra, ma computi 2 danni critici massimizzati ed esegui un'Azione in meno.

		\item \textbf{Furia Incontenibile} - Attiv.: \textbf{\emph{Pari}}. \emph{Effetto}: Confronta il Tiro per Colpire con un avversario adiacente per capire se l'hai colpito; se sì, somma anche un danno critico. \emph{Critico}: Fino alla fine del prossimo round hai -4 alla Difesa, +1d6 al Tiro per Colpire e ogni attacco andato a segno causa un danno critico aggiuntivo.
	\end{itemize}
...
\begin{narratore}[Partecipazione nel bene e nel male]
		Invitate il giocatore a creare un suo stile di \emph{fallimento}, fatelo gioire di un \emph{fumble}!
\end{narratore}
