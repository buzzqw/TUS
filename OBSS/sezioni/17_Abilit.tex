...
Le Abilità costituiscono una buona parte di ciò che può fare il personaggio, vanno scelte con attenzione e cura. È scegliendo le Abilità che si stabilisce lo stile e capacità del personaggio, se lo si vuole più guerriero o mago o curatore... o qualsivoglia combinazione e \emph{unicità}.
...
È possibile cambiare una Abilità scelta, rispettando comunque i requisiti, riaddestrandosi per almeno una settimana per 4 ore al giorno con qualcuno che abbia la nuova Abilità.\index{Riaddestrarsi}\index{Cambiare le Abilita'}
...
\subsection{Aggiungere nuove Abilità}\label{aggiungereabilita}
...
La \textbf{seconda volta} che prendi questa Abilità, requisito Lista Pugno Vuoto 6, Gru d'Argento 3, Pugno di Ferro 1, la tua distanza di mischia diventa di 3 metri.

La \textbf{terza volta} che prendi questa Abilità, requisito Lista Pugno Vuoto 12, Gru d'Argento 4, Pugno di Ferro 2, la tua distanza di mischia diventa di 4 metri.
...
\hspace{-0.2cm}\begin{tabularx}{\linewidth}{l@{\hspace{8pt}}X}
\rowcolor{gray!20}\textbf{Requisito}: & Lista d'Armi Pugno Vuoto 1, Competenza Armi 1, Costituzione 2, Saggezza 1\\
...
La \textbf{quarta} volta che si prende questa Abilità, requisito Competenza Armi 12, causi 4 danni critici aggiuntivi.
...
\emph{Livello Zatoichi}, la \textbf{terza volta} che prendi l'Abilità, requisito Consapevolezza a 5, in mischia una creatura invisibile non ha alcun vantaggio contro di te né tu hai penalità contro di lei.
...
La \textbf{prima volta} che prendi questa Abilità il costante e continuo allenamento ti permette di ridurre la penalità del multiattacco dato dall'attacco con l'arma secondaria. Quando attacchi con l'arma secondaria cumuli penalità al colpire di -4 al posto di -5 se l'arma è leggera.
...
La \textbf{prima volta} che prendi questa Abilità l'incantatore è in grado di infondere un incantesimo fino a livello 3 in un oggetto magico.

La \textbf{seconda volta} che prendi questa Abilità, requisito Competenza Magica 12, l'incantatore è in grado di infondere un incantesimo fino a livello 5 in un oggetto magico.

La \textbf{terza volta} che prendi questa Abilità, requisito Competenza Magica 16, l'incantatore è in grado di infondere un incantesimo fino a livello 8 in un oggetto magico.

La \textbf{quarta volta} che prendi questa Abilità, requisito Competenza Magica 18, l'incantatore è in grado di infondere un incantesimo fino a livello 9 in un oggetto magico.
...
La \textbf{terza volta}, requisito Armi Aggraziate 7, Intrattenere 5, che prendi l'Abilità puoi usare la Destrezza o il Carisma come modificatore al danno dell'arma, ignorando il danno dato dalla Forza.
...
La \textbf{prima volta} che prendi questa Abilità hai un +1 al Tiro per Colpire in mischia ed alla Difesa quando sei in mischia con 3 o più avversari.

La \textbf{seconda volta} che prendi questa Abilità hai un +2 al Tiro per Colpire in mischia ed alla Difesa quando sei in mischia con 2 o più avversari.
...
La \textbf{seconda volta} che prendi questa Abilità requisito Saggezza 3, Competenza Armi 9, puoi utilizzare fino al doppio del tuo punteggio in Saggezza per potenziare il Raggio Psichico.
...
La \textbf{quarta volta} che prendi questa Abilità consideri il totale dei dadi tirati come 10 se hai tirato da 4-9.
...
La \textbf{prima volta} che prendi questa Abilità puoi estrarre un'arma per te non grande con il costo di un'Azione immediata.
...
La \textbf{prima volta} che prendi questa Abilità ottieni un +2 alle prove di Natura ed un +2 ai Tiri Salvezza contro veleni naturali.

La \textbf{seconda volta} che prendi questa Abilità, requisito somma Tratti in comune 6, ottieni un +4 alle prove di Natura ed un +4 ai Tiri Salvezza contro effetti, anche magici, causati da Animali o Piante.
...
La \textbf{quarta volta} che prendi questa Abilità, requisito Animalia preso 4 volte, puoi trasformarti usando Animalia, in qualsiasi creatura purché non sia un immondo o drago.
...
La \textbf{prima volta} che prendi questa Abilità scegli una Lista Magica e due Trucchetti da questa lista. Puoi lanciare questi due Trucchetti senza Prova di Magia anche se sei Distratto o con armatura.
...
La \textbf{seconda volta} che prendi questa Abilità individui altri due Trucchetti, o se sei un Seguace o Devoto scegli un incantesimo di primo livello dalla Lista Privilegiata del Patrono. Requisito Tratto in comune a punteggio 3.

La \textbf{terza volta} che prendi questa Abilità individui altri due Trucchetti, o se sei un Devoto scegli un incantesimo di secondo livello od inferiore dalla Lista Privilegiata del Patrono. Requisito Tratto in comune a punteggio 5.

La \textbf{quarta volta} che prendi questa Abilità individui altri due Trucchetti, o se sei un Devoto scegli un incantesimo di terzo livello od inferiore dalla Lista Privilegiata del Patrono. Requisito Tratto in comune a punteggio 9.
...
La \textbf{seconda volta} che prendi questa Abilità, requisito somma tratti comuni con il Patrono Ledyal 8, ogni volta che usi un incantesimo di cura tu recuperi 1 Punto Ferita e la creatura curata +1d6 Punti Ferita in più.
...
La \textbf{prima volta} che prendi questa Abilità hai un +1 al Tiro per Colpire ed al Danno quando usi l'Arma preferita del tuo Patrono.
...
Questa Abilità si può usare nella Prova di Magia richiesta da \hyperlink{Guerriero della Magia}{Guerriero della Magia}. Le capacità indicate si cumulano.
...
La \textbf{terza volta} hai un +1d6 ai Tiri Salvezza contro i veleni magici e subire gli effetti di fumi tossici (ma puoi sempre soffocare).
...
La \textbf{terza volta} che prendi questa Abilità usando tre Punti Magia a round puoi concedere ad un'arma con cui vieni a contatto entrambe le Abilità precedenti.
...
La \textbf{seconda volta} che prendi questa Abilità, requisito Intrattenere 4, la forza della tua arte aggredisce i nemici e puoi selezionare due effetti tra: -2 Tiro per Colpire, -2 Danno in combattimento, -2 Difesa, -2 ai Tiri Salvezza su Volontà. I tuoi nemici devono essere entro 12 metri di raggio.
...
La \textbf{seconda volta} che prendi questa Abilità quando effettui una prova per saltare in lungo o in alto tiri 1d6 in più.
...
La \textbf{prima volta} che prendi questa Abilità se lo puoi vedere sai anche se è magico. Costa un'Azione attivare la vista magica e dura un round.
...
La \textbf{seconda volta} che prendi questa Abilità, requisito Competenza Armi 6 oppure Pugno Vuoto 4, usi un'Azione Immediata per Preparare la Difesa.
...
La \textbf{seconda volta} che prendi questa Abilità sul medesimo terreno, specializzandoti, acquisisci una capacità a seconda del terreno.
...
\feat{Piu' sono grossi più fanno rumore quando cadono}
\hspace{-0.2cm}\begin{tabularx}{\linewidth}{l@{\hspace{8pt}}X}
\rowcolor{gray!20}\textbf{Requisito}: & Competenza Armi 1\\
\textbf{Tiri Salvezza}: & +2 Tempra, +1 Volontà\\
\rowcolor{gray!20}\textbf{Caratteristica}: & Forza o Carisma\\
...
La \textbf{prima volta} che prendi questa Abilità il danno causato dai tuoi colpi ed il Tiro per Colpire aumentano di 1.
...
La \textbf{prima volta} che prendi questa Abilità come Reazione ad un'Azione di attacco avversaria puoi aggiungere +1 alla tua Difesa. Puoi usare l'Abilità fino a 3 volte al giorno.
...
La \textbf{terza volta} che prendi l'Abilità, requisito Competenza Armi 8, Destrezza 4, fino a due avversari non prendono il bonus al colpire dato da fiancheggiamento.
...
La \textbf{prima volta} che prendi questa Abilità le penalità alle prove di competenza di Base date dall'armatura diminuiscono di 1.

La \textbf{seconda volta} che si prende questa Abilità, requisito Competenza Armi 6, la penalità alle prove di competenza diminuisce di ulteriore 1. La penalità al movimento diminuisce di 1 metro. Puoi dormire in armature medie senza essere affaticato.

La \textbf{terza volta} che si prende questa Abilità, requisito Competenza Armi 11, la penalità alle prove di competenza diminuisce di ulteriore 1. La penalità al movimento diminuisce di un ulteriore 1 metro. Puoi dormire in armature pesanti senza essere affaticato.
...
La capacità di non lasciare impronte nell'ambiente scelto. Ogni volta che prendi questa Abilità puoi scegliere un ambiente diverso (vedi Abilità \hyperlink{Passo Sicuro}{Passo Sicuro} di cui hai preso l'Abilità. La difficoltà della prova di Seguire Tracce per inseguirti è aumentata di 10.
...
La \textbf{quinta volta} che prendi questa Abilità, requisito Competenza Magica 17, nella Lista di Magia \emph{preferita} ogni qual volta devi tirare una Prova di Magia puoi non tirare e considerare di aver fatto due Successi Critici Magici.
...
\begin{itemize}[leftmargin=*] \setlength{\itemsep}{0pt}
\item Ogni volta che l'Abilità viene presa, oltre la prima, si devono selezionare due nuove Liste di Magia \emph{opposte} ed il costo di lancio per gli incantesimi di queste liste aumenta di 1.  La Lista di Magia Universale non è sceglibile tra le \emph{opposte}.

\item L'Abilità \emph{Un solo credo} non può essere presa assieme a: \hyperlink{Figlio Unico}{Figlio Unico}, \hyperlink{Magie Potenti}{Magie Potenti}, \hyperlink{Specialista}{Specialista}.

\item Se usi l'Abilità \emph{Un solo credo} non puoi usare le \hyperlink{abilitadilista}{Abilità di Lista} (pag. \pageref{abilitadilista}).
...
La \textbf{prima volta} che prendi questa Abilità ottieni un bonus di +2 ai Tiri Salvezza su Volontà. Il bonus è cumulativo, +2 la prima volta, +1 la \textbf{seconda volta}, +1 la \textbf{terza} volta.
...
\pagebreak
	...
	\newcommand{\featlink}[1]{\hyperlink{#1}{#1}}
	\newcommand{\featlinktext}[2]{\hyperlink{#1}{#2}}
...
\textbf{{\large Competenza Armi}}\\
	...
	\textbf{CA 3:} \featlink{Colpo Furtivo}, \featlink{Lesto}, \featlink{Occhio Clinico}, \featlink{Occhio di Falco}, \featlink{Parata}, \featlink{Spara e Scappa} (Lista Balestre 3), \featlink{Stai giù!}\\

	\textbf{CA 4:} \featlink{Doppia porzione} (\featlink{Combattimento con due armi}), \featlink{Ho detto CADI!}, \featlink{Testa cava} (Lista Balestre 4), \featlink{Uno con l'arco}, \featlink{Ferocia} (2), \featlink{Precisino} (2), \featlink{Schivata prodigiosa} (2)\\

	\textbf{CA 5:} \featlink{Colpo Mortale}, \featlink{Forgiato nella furia}, \featlink{Questa è la mia arma!} (2), \featlink{Schivare trappole} (2)\\

	\textbf{CA 6:} \featlink{Danno Coordinato} (Saggezza 2), \featlink{Lesto} (2), \featlink{Proseguire} (2), \featlink{Armato} (2), \featlink{Colpo Furtivo} (2), \featlink{La mia pelle} (2), \featlink{Iaijutsu} (2), \featlink{Parata} (2), \featlink{Seconda pelle} (2), \featlink{Tattico} (2, Intelligenza 2)\\

	\textbf{CA 7:} \featlink{Ferocia} (3), \featlink{Uno con l'arco} (2, Destrezza 3)\\

	\textbf{CA 8:} \featlink{Danno Coordinato} (2, Intelligenza 2), \featlink{Schivata prodigiosa} (3, Destrezza 4)\\

	\textbf{CA 9:} \featlink{Parata} (3), \featlink{Artista dell'Arma} (2), \featlinktext{Un braccio, un'arma}{Un braccio un'arma} (2)\\

	\textbf{CA 10:} \featlink{Colpo Furtivo} (3), \featlink{Segugio} (2, Saggezza 2)\\

	\textbf{CA 11:} \featlink{Ferocia} (4), \featlink{Seconda pelle} (3)\\

	\textbf{CA 12:} \featlink{Attacco Turbinante} (Intrattenere 3), \featlink{Colpo Indebolente} (\featlinktext{Colpo Furtivo}{Colpo furtivo} 3), \featlink{Combattimento con due armi} (2, Destrezza 3), \featlink{Colpo Furtivo} (4), \featlink{Iaijutsu} (3), \featlink{Pelle Coriacea} (3, Costituzione 3)\\

	\textbf{CA 13:} \featlink{Artista dell'Arma} (3), \featlinktext{Un braccio, un'arma}{Un braccio un'arma} (3)\\

	\textbf{CA 15:} \featlink{Attacco Turbinante} (2, Intrattenere 5)\\

	\textbf{CA 16:} \featlink{Segugio} (3, Saggezza 3), \featlink{Pelle Coriacea} (4)\\

	\textbf{CA 18:} \featlink{Colpo Paralizzante} (\featlink{Colpo Indebolente}, \featlinktext{Colpo Furtivo}{Colpo furtivo} 4), \featlink{Combattimento con due armi} (3)\\
...
\textbf{{\large Competenza Magica}}\\
	...
	\textbf{CM 2:} \featlink{Concentrato} (ripetibile CM/4), \featlink{Un solo credo}, \featlink{Animalia} (1) - Bestie (Seguace/Devoto Efrem/Shayalia), \featlink{Sangue Puro} (\featlink{Animalia}, Devoto Efrem/Shayalia)\\

	\textbf{CM 3:} \featlink{Batteria Magica} (ripetibile CM/3), \featlink{Elementalista}, \featlink{Prodigioso} (ripetibile CM/4), \featlink{Specialista} (ripetibile), \featlink{Figlio Unico} (2, Tratto comune 3)\\

	\textbf{CM 4:} \featlink{Animalia} (2) - Piante e Melme, \featlink{Imposizione delle mani} (2, Tratti 4), \featlink{Infondere Energia Magica} (2)\\

	\textbf{CM 5:} \featlink{Magie Potenti} (ripetibile CM/4), \featlink{Il Patrono è la mia Arma} (2, Tratti 5, CA 1), \featlink{Figlio Unico} (3, Tratto comune 5)\\

	\textbf{CM 6:} \featlink{Creare Oggetti Magici} - livello 3, \featlink{Dadi Truccati}, \featlink{Sifone Nero} (Seguace/Devoto Tazher, Tratti 6), \featlink{Elementalista} (2), \featlink{Il Patrono è con me} (2, Tratti 6), \featlink{Imposizione delle mani} (3, Tratti 6), \featlink{Un solo credo} (2), \featlink{Incantatore da Combattimento} (2), \featlink{Figlia di Shayalia} (2, Tratti comuni 6), \featlink{Forma Elementale} (Seguace/Devoto, 3 incantesimi 2 Liste Elementali)\\

	\textbf{CM 7:} \featlink{Infondere Energia Magica Superiore} (2)\\

	\textbf{CM 8:} \featlink{Incantatore Prudente}, \featlink{Infondere Energia Magica} (3), \featlink{Imposizione delle mani} (4, Tratti 8), \featlink{Sangue Puro} (2), \featlink{Armatura del Devoto} (3, Tratto singolo 8)\\

	\textbf{CM 9:} \featlink{Figlio Unico} (4, Tratto comune 9)\\

	\textbf{CM 10:} \featlink{Animalia} (3) - Elementali, \featlink{Imposizione delle mani} (5, Tratti 10), \featlink{Il Patrono è la mia Arma} (3, Tratti 10, CA 4), \featlink{Il Patrono è la mia Arma} (4, Tratti 10, CA 5)\\

	\textbf{CM 11:} \featlink{Forma Elementale} (2), \featlink{Un solo credo} (3)\\

	\textbf{CM 12:} \featlink{Creare Oggetti Magici} (2) - livello 5, \featlink{Dadi Truccati} (2), \featlink{Il Patrono è con me} (3, Tratti 12), \featlink{Imposizione delle mani} (6, Tratti 12), \featlink{Incantatore da Combattimento} (3), \featlink{Sangue Puro} (3), \featlink{Figlia di Shayalia} (3, Tratti comuni 12), \featlink{Armatura del Devoto} (4, Tratto singolo 12)\\

	\textbf{CM 13:} \featlink{Il Patrono è la mia Arma} (5, Tratti 13, CA 6)\\

	\textbf{CM 14:} \featlink{Forma Elementale} (3), \featlink{Un solo credo} (4)\\

	\textbf{CM 16:} \featlink{Creare Oggetti Magici} (3) - livello 8, \featlink{Animalia} (4) - Mostruosità, \featlink{Il Patrono è la mia Arma} (6, Tratti 16)\\

	\textbf{CM 17:} \featlink{Un solo credo} (5)\\

	\textbf{CM 18:} \featlink{Creare Oggetti Magici} (4) - livello 9\\

	\textbf{CM 20:} \featlink{Un solo credo} (6)\\

\textbf{{\large Abilità miste (CA/CM)}}\\

	\textbf{CA 1, CM 1:} \featlink{Lo scudo è mio amico}, \featlink{Energia Psichica}, \featlink{Armatura della Montagna Incantata} (Lista Pugno Vuoto 1, Costituzione 2, Saggezza 1)\\

	\textbf{CA 1, CM 2:} \featlink{Infondere Energia Magica}\\

	\textbf{CA 2, CM 2:} \featlink{Guerriero della Magia}\\

	\textbf{CA 3, CM 3:} \featlink{Guerriero della Magia} (2)\\

	\textbf{CA 4, CM 4:} \featlink{Energia Psichica} (2)\\

	\textbf{CA 4, CM 6:} \featlink{Infondere Energia Magica Superiore}\\

	\textbf{CA 6, CM 9:} \featlink{Guerriero della Magia} (3)\\

	\textbf{CA 9, CM 12:} \featlink{Guerriero della Magia} (4)\\

	\textbf{Catene derivate (richiedono abilità precedenti):}
	\begin{itemize}
		\item \featlink{Colpo Psichico} (da \featlink{Energia Psichica}, Destrezza 1): livelli con CA 2, CA 3, CA 7 (Saggezza 4)
		\item \featlink{Raggio Psichico} (da \featlink{Colpo Psichico}, Saggezza 3): livelli con CA 5, CA 9
	\end{itemize}

\textbf{{\large Patroni}}\\

	\textbf{\featlink{Armatura del Devoto}:} Tratto singolo 4 (livello 1), Tratto singolo 6 (livello 2), Tratto singolo 8 (livello 3), Tratto singolo 12 (livello 4), essere Devoto o Seguace\\

	\textbf{\featlink{Figlia di Shayalia}:} Devoto/Seguace Shayalia (livello 1), Tratti comuni 6 (livello 2), Tratti comuni 12 (livello 3), \featlink{Animalia} preso 4 volte (livello 4)\\

	\textbf{\featlink{Figlio di Tazher}:} Devoto/Seguace Tazher, Tratti 10\\

\textbf{{\large Liste d'armi specifiche}}\\

	\textbf{Lista Pugno Vuoto:}
	\begin{itemize}
		\item \featlink{Armatura della Montagna Incantata} (PV 1, CA 1, Costituzione 2, Saggezza 1): 5 livelli progressivi (PV 5, 7, 8, 13)
		\item \featlink{Gru d'Argento} (PV 2, Destrezza 1): 5 livelli progressivi (PV 4, 9+Des2, 11, 13+Des3)
		\item \featlink{Ali della Fenice} (PV 3, \featlink{Gru d'Argento} 1): 3 livelli progressivi (PV 6+Gru3+PF1, PV 12+Gru4+PF2)
		\item \featlink{Pugno di Ferro} (PV 3): 6 livelli progressivi (PV 6, 9, 12, 15, 18)
		\item \featlink{Pugno Potente} (PV 3)
		\item \featlink{Tempesta di Furia} (PV 7, Destrezza 1, Forza 1)
		\item \featlink{Parata} (CA 3 oppure PV 2): livelli con CA 6/PV 4, CA 9/PV 6
	\end{itemize}

	\textbf{Lista d'Armi Aggraziate:}
	\begin{itemize}
		\item \featlink{Danza della Lama} (AA 2, Destrezza/Carisma 1, Intrattenere 1): 3 livelli progressivi (AA 4+Int3, AA 7+Int5)
	\end{itemize}

	\textbf{Lista Balestre:}
	\begin{itemize}
		\item \featlink{Spara e Scappa} (Balestre 3)
		\item \featlink{Testa cava} (Balestre 4)
	\end{itemize}

	\textbf{Lista Palle Rotanti:}
	\begin{itemize}
		\item \featlink{Flagello Danzante} (CA 1, Lista Palle Rotanti)
	\end{itemize}
	...
	\textbf{Costituzione -1:} \featlink{Robusto} (2 livelli)\\
	\textbf{Costituzione 0:} \featlink{Finta Morte}, \featlink{Guarigione accelerata} (3 livelli), \featlink{Recupero}, \featlink{Resistenza della pietra} (4 livelli)\\
	\textbf{Costituzione 1:} \featlink{Colosso} (4 livelli), \featlinktext{Immunita' ai veleni}{Immunità ai veleni} (3 livelli), \featlink{Rinoceronte} (3 livelli)\\
	\textbf{Costituzione 2:} \featlink{Pelle Coriacea} (4 livelli, poi serve CA)\\
	\textbf{Destrezza 1:} \featlink{Riflessi fulminei} (4 livelli), \featlink{Iniziativa migliorata} (2 volte), \featlink{Passo rapido} (ripetibile)\\
	\textbf{Destrezza 2:} \featlink{Giocoliere}\\
	\textbf{Destrezza 3:} \featlink{Schivare trappole} (2 livelli, poi serve CA 5)\\
	\textbf{Forza 0:} \featlink{Molla} (2 livelli)\\

	\textbf{Caratteristiche Mentali}

	\textbf{Saggezza 0:} \featlink{Percettivo} (max 3 volte), \featlinktext{Volonta' Ferrea}{Volontà Ferrea} (4 livelli)\\
	\textbf{Saggezza 1:} \featlink{Guaritore} (2 livelli), \featlink{Muro mentale} (ripetibile), \featlink{Passo Sicuro} (2 livelli + \featlink{Senza Traccia})\\
	\textbf{Intelligenza -1:} \featlink{Poliglotta} (solo creazione)\\

	\textbf{Competenze Specifiche}

	\textbf{Caratteristica -1:} \featlink{Esperto} (4 livelli)\\
	\textbf{Consapevolezza 2:} \featlink{Combattere alla Cieca} (3 livelli: Cons 2, 3, 5)\\
	\textbf{Conoscenza 1:} \featlink{Conoscenza istintiva}\\
	\textbf{Cavalcare 1:} \featlink{Difendere Cavalcatura}\\
	\textbf{Carisma 2:} \featlink{Infondere Coraggio} (Intrattenere 1), \featlink{Infondere Paura}\\
	\textbf{Intrattenere 4:} \featlink{Infondere Coraggio} (2), \featlink{Infondere Paura} (2)\\
	\textbf{Intrattenere 6:} \featlink{Litania versatile} (2 livelli)\\
	\textbf{Intrattenere 12:} \featlink{Infondere Coraggio} (3), \featlink{Infondere Paura} (3)\\
	\textbf{Furtività 1:} \featlink{Passo Felpato} (2 livelli: Fur1, poi Des3+Fur8)\\
	\textbf{Tratti Ledyal 8:} \featlink{Guaritore} (2)\\
	...
	\textbf{\featlink{Fortunato} + 6 punti Tratti:} \featlink{Sfortunato}\\
	\textbf{\featlink{Passo Sicuro}:} \featlink{Senza Traccia} (per ogni ambiente)\\
...
\subsection{Raggruppamento Abilità per Stile}
...
\hyperlink{Guarigione accelerata}{Guarigione accelerata} \textit{(Cos 0)}\\
...
\hyperlink{La mia Testa è più Dura}{La mia Testa è più Dura} \textit{(CA 1)}\\
...
\hyperlink{Piu' sono grossi più fanno rumore quando cadono}{Piu' sono grossi più fanno rumore quando cadono} \textit{(CA 1)}\\
...
\hyperlink{Un solo corpo, una sola mente, un solo spirito}{Un solo corpo, una sola mente, un solo spirito} \textit{(Nessun requisito)}\\
...
\hyperlink{Questo è il mio pugnale}{Questo è il mio pugnale} \textit{(CA 1)}\\
...
\hyperlink{Arciere su saurovallo}{Arciere su saurovallo} \textit{(CA 1)}\\
...
13	&	13	& 0	&	13d6+39	&\hyperlink{Volonta' Ferrea}{Volontà Ferrea}\\
...
& \centering\arraybackslash \textbf{CA} & \centering\arraybackslash \textbf{CM} & \centering\arraybackslash \textbf{PF} & \\
 ...
 \rowcolor{gray!20}14	&	14	& 0	&	14d6+42	&\\
	15	&	15	& 0	&	15d6+45	&\hyperlink{Piu' sono grossi più fanno rumore quando cadono}{Piu' sono grossi più fanno rumore quando cadono}\\
...
\noindent\begin{tabularx}{\linewidth}{p{0.3cm}|p{0.3cm}p{0.3cm}c|X|}
%\noindent\begin{tabularx}{\linewidth}{c|>{\hsize=0.08\hsize}X>{\hsize=0.08\hsize}X>{\hsize=0.33\hsize}c|X|}
	\toprule
...
& \centering\arraybackslash \textbf{CA} & \centering\arraybackslash \textbf{CM} & \centering\arraybackslash \textbf{PF} & \\
 ...
 \rowcolor{gray!20}6	&	6	& 0	&	6d6+18	&\hyperlink{Opportunista}{Opportunista}\\
	7	&	7	& 0	&	7d6+21	&\hyperlink{Volonta' Ferrea}{Volontà Ferrea}\\
 \rowcolor{gray!20}8	&	8	& 0	&	8d6+24	&\\
...
& \centering\arraybackslash \textbf{CA} & \centering\arraybackslash \textbf{CM} & \centering\arraybackslash \textbf{PF} & \\
 ...
 \rowcolor{gray!20}10	&	5	& 5	&	10d6+15	&\hyperlink{Raggio Psichico}{Raggio Psichico}\\
	11	&	6	& 5	&	11d6+18	&\\
 \rowcolor{gray!20}12	&	6	& 6	&	12d6+18	&\hyperlink{Volonta' Ferrea}{Volontà Ferrea}\\
