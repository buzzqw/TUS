...
I racconti si fanno molto confusi a questo punto; di fatto, l'\emph{Omniessenza} era effettivamente qualcosa di vero e di \emph{vivo}, una parte di un'energia più grande. All'attivazione del reattore questo esplose con un'energia e una forza mai viste sulla Terra; buona parte di quelli che erano gli stati centrali degli USA venne vaporizzata all'istante.
...
Da questa fiamma uscì un mastodontico \emph{drago rosso} le cui scaglie erano impenetrabili a qualsiasi arma umana. Tàhil, questo il suo nome, nelle prime 24 ore distrusse quello che restava della costa orientale degli Stati Uniti.

All'alba del secondo giorno una mistica energia avvolse Tàhil e questo mutò in un drago a molte teste di colori diversi. Nel suo petto apparve un'altra breccia e a ogni passo una moltitudine di altri draghi, per fortuna \emph{leggermente} più piccoli, cominciò a uscire.
...
Ti elenco un po' di termini\index{Termini comuni} e concetti che troverai ripetuti più volte nel libro.
...
\textbf{Azione}: \index{Azione} è ciò che si fa in un intervallo di tempo. Ogni cosa che viene fatta dal personaggio si misura in Azioni. Combattere, lanciare Incantesimi, scassinare, bere pozioni, spostarsi... in ogni round si possono fare 3 Azioni. Un'Azione dura circa 3 secondi.
...
\textbf{Lanciare Incantesimi sotto distrazione o dopo una ferita recente}:\index{Prova di Concentrazione}\index{Lanciare Incantesimi sotto distrazione o dopo una ferita recente}\index{Distratto} quando un incantatore vuole usare una Magia ma è disturbato, è stato ferito nello stesso round o comunque distratto durante il lancio di un incantesimo allora dovrà effettuare una Prova di Magia.
...
\textbf{Competenza} \index{Competenza}(skill)\index{Skill}: la competenza ci dice ciò che sappiamo ed il suo valore indica il grado di conoscenza della stessa. Può essere lo studio di una lingua, l'arrampicarsi, il notare piccole cose.
...
\textbf{+1d6 oppure -1d6}: è un bonus o una penalità a una prova. Aggiungi o sottrai un dado a 6 alla prova. Il massimo della penalità porta il numero dei dadi lanciati a 0 e il bonus massimo a +3d6.\index{Massimo valore del bonus}

\textbf{Distanza}:\index{Distanza} la distanza, per quanto riguarda il combattimento, è misurata in quadretti da 1 metro.

\textbf{Devoto}\index{Devoto}: un personaggio che si è legato a un Patrono e ha almeno 2 Tratti in comune.

\textbf{Seguace}: un personaggio che si è legato ad un Patrono con 1 Tratto in comune
...
\textbf{Iniziativa}: \index{Iniziativa}è una prova di Destrezza oppure Intelligenza. Stabilisce l'ordine delle azioni in combattimento. Chi ha il punteggio più alto nella prova agisce per primo.

\textbf{Livello}:\index{Livello} il Livello indica la competenza e potere raggiunto dal personaggio. Può indicare quanto è \emph{forte} il nemico.
...
\textbf{Mischia}: \index{Mischia}con mischia si intende il combattimento di contatto, corpo a corpo, spada a spada, ovvero quando il tuo personaggio combatte con un'arma che non abbia gittata (arco, balestre, fionde...) contro un avversario.
...
\textbf{Movimento}: \index{Movimento}il movimento rappresenta la capacità di spostarsi. Un'Azione di Movimento rappresenta lo spostamento del personaggio: più è alto il valore di Movimento, più metri una creatura può muoversi.
...
\textbf{Punteggi caratteristica}: \index{Punteggi caratteristica} \index{Statistiche} abbreviati anche in caratteristica o statistiche. Ogni personaggio ha 6 Caratteristiche: Forza (FOR), Destrezza (DES), Costituzione (COS), Intelligenza (INT), Saggezza (SAG) e Carisma (CAR). Più è alto il punteggio, maggiore è la capacità del personaggio in quello specifico ambito.
...
\textbf{Punti ferita (Punti Ferita)}:\index{Punti Ferita} indicano l'energia vitale, la resistenza e la fortuna nel resistere alle ferite della creatura. Finché la creatura ha almeno 1 Punto Ferita, combatterà al suo meglio, senza problemi (ma potrebbe anche decidere di scappare piuttosto che morire!).
...
\textbf{Riduzione del Danno (DR)}: \index{Riduzione del Danno} \index{DR} alcune creature hanno una resistenza innata ai danni e alle ferite. Questa resistenza si denota come DR. La Riduzione si applica dopo la Resistenza e i Tiri Salvezza.
...
\textbf{Tiro Salvezza (TS)}:\index{Tiro Salvezza} \index{TS}quando una creatura è sottoposta a un effetto particolare, spesso viene concesso un Tiro Salvezza per mitigarne o annullarne gli effetti. Il Tiro Salvezza è un'azione che non occupa tempo o Azioni.
...
\textbf{Uno porta male}: \index{Uno porta male}se tiri un 1 con il dado, togli 1 dal risultato totale. Non per questo un 6 tirato diventa un 5: l'esplosione del 6 rimane, solo che togli 1 al risultato finale. Detta diversamente, 1 vale 0.
...
\begin{giocatore}[Informazioni per il Giocatore]Box contenente indicazioni e chiarimenti per il Giocatore. Riconoscibile dall'icona $\dagger$\end{giocatore}

\begin{narratore}[Informazioni per il Narratore]Box contenente indicazioni e suggerimenti per il Narratore. Riconoscibile dalla icona {\faBook}\end{narratore}
