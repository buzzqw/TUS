 ...
 * Versione migliorata con gestione del limite di 512KB di GitHub Wiki e navigazione tra sezioni
 * 
...
📝 DESCRIZIONE:
   ...
   ✨ NOVITÀ v1.3.0: Navigazione automatica tra sezioni con link "Prossima sezione"!
...
📤 COSA FA IN MODALITÀ NORMALE:
   ...
   • Aggiunge navigazione automatica tra sezioni consecutive
...
🧭 NAVIGAZIONE AUTOMATICA:
   • Ogni sezione include un link "Prossima sezione ➡️" alla fine
   • L'ultima sezione include un link di ritorno all'indice
   • La navigazione è separata per lingua (italiano/inglese)
...
`);
    ...
    // Nuovo metodo per aggiungere la navigazione alle sezioni
    // Metodo aggiornato per aggiungere la navigazione completa alle sezioni
    addNavigationToSection(sectionContent, currentIndex, sectionFiles, language, indexFileName) {
        const isItalian = language === 'ita';
        let navigationContent = '\n\n---\n\n';
        
        // Crea una riga di navigazione con tutti i link disponibili
        const navLinks = [];
        
        // Link alla sezione precedente
        if (currentIndex > 0 && sectionFiles[currentIndex - 1]) {
            const prevSectionFile = sectionFiles[currentIndex - 1];
            const prevLink = isItalian 
                ? `[⬅️ Sezione precedente](${prevSectionFile})`
                : `[⬅️ Previous section](${prevSectionFile})`;
            navLinks.push(prevLink);
        }
        
        // Link di ritorno all'indice (sempre presente)
        const indexLink = isItalian 
            ? `[📋 Indice](${indexFileName})`
            : `[📋 Index](${indexFileName})`;
        navLinks.push(indexLink);
        
        // Link alla sezione successiva
        if (currentIndex < sectionFiles.length - 1 && sectionFiles[currentIndex + 1]) {
            const nextSectionFile = sectionFiles[currentIndex + 1];
            const nextLink = isItalian 
                ? `[Prossima sezione ➡️](${nextSectionFile})`
                : `[Next section ➡️](${nextSectionFile})`;
            navLinks.push(nextLink);
        }
        
        // Unisce i link con separatori
        if (navLinks.length > 0) {
            navigationContent += '**Navigazione: ' + navLinks.join(' | ') + '**\n\n';
        }
        
        // Aggiunge informazioni aggiuntive sulla posizione
        const positionInfo = isItalian 
            ? `*Sezione ${currentIndex + 1} di ${sectionFiles.length}*`
            : `*Section ${currentIndex + 1} of ${sectionFiles.length}*`;
        navigationContent += positionInfo;
        
        return sectionContent + navigationContent;
    }
    ...
    async createIndexPage(language, sections, sectionFiles) {
        ...
        indexContent += `\n${isItalian ? '**Nota**: Le sezioni molto grandi sono state automaticamente divise in più parti per rispettare i limiti di GitHub Wiki (512KB). Ogni sezione include navigazione automatica alla sezione successiva.' : '**Note**: Very large sections have been automatically split into multiple parts to comply with GitHub Wiki limits (512KB). Each section includes automatic navigation to the next section.'}`;
...
`;
    ...
    async processFile(filename, language) {
        ...
        // Prima crea la pagina indice per ottenere il nome del file
        const indexFileName = `${this.today}-OBSSv2${language === 'eng' ? '-eng' : ''}`;
        const sectionFiles = [];
        
        // Crea file per ogni sezione con navigazione
        for (let i = 0; i < sections.length; i++) {
            ...
            // Prepara l'array dei nomi file per la navigazione
            const tempSectionFiles = [];
            for (let j = 0; j < sections.length; j++) {
                const tempSanitizedTitle = this.sanitizeFileName(sections[j].title);
                tempSectionFiles.push(`${this.today}-${tempSanitizedTitle}${language === 'eng' ? '-eng' : ''}`);
            }
            
            // Aggiungi navigazione al contenuto della sezione
            const sectionWithNavigation = this.addNavigationToSection(
                section.content, 
                i, 
                tempSectionFiles, 
                language, 
            ...
            const success = await this.createWikiFile(fileName, sectionWithNavigation);
    ...
    async run() {
        ...
        console.log('🧭 Navigazione automatica tra sezioni: ATTIVA');
        ...
        try {
            ...
            if (itaFiles.length > 0) {
                ...
                console.log('✓ File italiano elaborato con successo (con navigazione)');
            } else {
            ...
            if (engFiles.length > 0) {
                ...
                console.log('✓ File inglese elaborato con successo (con navigazione)');
            } else {
            ...
            console.log('🧭 Ogni sezione include navigazione automatica alla sezione successiva!');
