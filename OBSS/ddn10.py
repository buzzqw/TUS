...
class LaTeXCompiler:
    ...
    def needs_compilation(self, tex_file):
        """Verifica se compilazione è necessaria usando solo timestamp"""
        ...
        try:
            # Usa solo i timestamp dei file, non leggere il contenuto
            ...
            # Controlla anche file correlati (bibliografia)
            ...
            # In caso di errore, forza compilazione per sicurezza
            return True
    
    def detect_encoding(self, file_path):
        """Rileva l'encoding del file leggendo solo i primi byte"""
        encodings = ['utf-8', 'latin1', 'cp1252', 'iso-8859-1', 'utf-16', 'utf-32']
        
        # Prima prova a leggere solo un campione del file
        try:
            with open(file_path, 'rb') as f:
                raw_data = f.read(8192)  # Leggi solo i primi 8KB
        except Exception as e:
            print_msg("❌", f"Errore lettura raw {file_path.name}: {e}")
            return 'latin1'
        
        for encoding in encodings:
            try:
                raw_data.decode(encoding)
                print_msg("🔤", f"Encoding rilevato per {file_path.name}: {encoding}")
                return encoding
            except UnicodeDecodeError:
                continue
        
        print_msg("⚠️", f"Impossibile rilevare encoding per {file_path.name}, uso latin1")
        return 'latin1'  # Fallback sicuro
    
    def read_file_safe(self, file_path):
        """Legge file con encoding detection sicuro"""
        encoding = self.detect_encoding(file_path)
        try:
            with open(file_path, 'r', encoding=encoding, errors='replace') as f:
                content = f.read()
            print_msg("📖", f"File {file_path.name} letto con encoding {encoding}")
            return content, encoding
        except Exception as e:
            print_msg("❌", f"Errore lettura {file_path.name}: {e}")
            # Ultimo tentativo con latin1 e replace
            try:
                with open(file_path, 'r', encoding='latin1', errors='replace') as f:
                    content = f.read()
                print_msg("🔄", f"Fallback latin1 per {file_path.name}")
                return content, 'latin1'
            except Exception as e2:
                print_msg("💥", f"Errore fatale lettura {file_path.name}: {e2}")
                return None, None
    
    def write_file_safe(self, file_path, content, encoding='utf-8'):
        """Scrive file con encoding sicuro"""
        try:
            with open(file_path, 'w', encoding=encoding) as f:
                f.write(content)
            return True
        except Exception as e:
            print_msg("❌", f"Errore scrittura {file_path.name}: {e}")
            return False
    
    def generate_bib(self, tex_file):
        """Genera file .bib se necessario con gestione robusta"""
        if tex_file.name == "OBSSv2-eng.tex":
        ...
        try:
            ...
            # Usa subprocess senza text mode per evitare problemi encoding
            result = subprocess.run(
                [sys.executable, str(converter), str(bib_source)],
                capture_output=True,
                timeout=30,
                env=env
                # Non specificare text=True o encoding
            )
            
            # Decodifica output manualmente
            try:
            ...
            if result.returncode == 0:
                ...
                print_msg("⚠️", f"Errore generazione bibliografia (exit {result.returncode})")
                if stderr_text.strip():
                    print_msg("📋", f"Errore: {stderr_text.strip()[:100]}")
                return False
                
        except subprocess.TimeoutExpired:
            print_msg("⏰", "Timeout generazione bibliografia")
            return False
        except Exception as e:
...
'''
    ...
    def compile_single_file(self, tex_file):
        """Compila un singolo file con gestione encoding robusta"""
        ...
        try:
            ...
            env = os.environ.copy()
            env.update({
                'TEXMFCACHE': str(self.aux_dir / 'texmf'),
                'BIBER_CACHE': str(self.aux_dir / 'biber'),
                'LANG': 'C.UTF-8',
                'LC_ALL': 'C.UTF-8',
                'PYTHONIOENCODING': 'utf-8:replace'
            })
            ...
            print_msg("🔧", f"Comando: {' '.join(cmd[:4])}")
            
            # SOLUZIONE: Non catturare text output, solo bytes
            result = subprocess.run(
                cmd,
                cwd=tex_file.parent,
                capture_output=True,
                ...
                env=env
                # NON specificare encoding o text=True
            )
            
            duration = time.time() - start_time
            pdf_file = tex_file.with_suffix('.pdf')
            
            # Decodifica output in modo sicuro
            try:
                stdout_text = result.stdout.decode('utf-8', errors='replace')
            except:
                stdout_text = str(result.stdout)
            ...
            if result.returncode == 0 and pdf_file.exists():
                ...
                print_msg("❌", f"Compilazione {tex_file.name} fallita dopo {duration:.2f}s (exit code: {result.returncode})")
                
                # Log errori con gestione sicura
                ...
                # Cerca errori specifici nel log LaTeX
                ...
                if log_file.exists():
                    try:
                        ...
                        # Cerca errori LaTeX comuni
                        lines = log_content.split('\n')
                        error_patterns = ['! ', 'Error:', 'Fatal error', '! LaTeX Error']
                        
                        for line in lines:
                            for pattern in error_patterns:
                                if pattern in line:
                                    print_msg("📋", f"LaTeX: {line.strip()[:120]}")
                                    break
                    except Exception as e:
                        print_msg("⚠️", f"Errore lettura log: {e}")
                
                return False, duration, {'error': stderr_text[:500], 'exit_code': result.returncode}
