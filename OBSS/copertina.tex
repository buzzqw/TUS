...
\tikz[remember picture,overlay]  \node[opacity=1, xshift=(\paperwidth)*0.5+3.6cm,yshift=0.35cm,inner sep=0pt] at (current page.east){\includegraphics[width=21.0cm,height=\pdfpageheight]{immagini/copertina2-ai.png}};


%\tikz[remember picture,overlay] \node[opacity=0.3,inner sep=0pt] at (current page.center){\includegraphics[width=\paperwidth,height=\paperheight]{example-image}};

\begin{textblock*}{15cm}(33.5cm,7cm) % {block width} (coords)
\Huge {Old Bell School System}\\
\end{textblock*}

\begin{textblock*}{20cm}(37cm,8cm) % {block width} (coords)
\Large {\textbf{(OBSS)}}\\
\end{textblock*}
