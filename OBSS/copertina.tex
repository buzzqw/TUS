...
\tikz[remember picture,overlay]  \node[opacity=0.3, xshift=(\paperwidth)*0.5+4cm,yshift=0.35cm,inner sep=0pt] at (current page.east){\includegraphics[width=\paperwidth,height=\pdfpageheight]{copertina.png}};


%\tikz[remember picture,overlay] \node[opacity=0.3,inner sep=0pt] at (current page.center){\includegraphics[width=\paperwidth,height=\paperheight]{example-image}};

\begin{textblock*}{20cm}(28.5cm,3cm) % {block width} (coords)
\Huge \textbf{Old Bell School System (OBSS)}\\

\end{textblock*}

\begin{textblock*}{20cm}(31cm,4.5cm) % {block width} (coords)
%{\Huge \textit{Adventures Game}}\\
\calligra\Huge{Adventures Game}\\

\end{textblock*}

\begin{textblock*}{20cm}(34.5cm,6cm) % {block width} (coords)
		{\LARGE \versione}
\end{textblock*}
