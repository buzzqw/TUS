...
\documentclass[a4,openany]{dndbook}
\usepackage{multicol}
\usepackage{lmodern}
...
\pagestyle{plain} 
\begin{quotebox}\textit{Si puo' scoprire di piu' su una persona in un'ora di gioco che in un anno di conversazione. (Platone)}
\end{quotebox}
...
\begin{quotebox}\textit{Il vero viaggio di scoperta non consiste nel trovare nuovi territori, ma nel possedere altri occhi, vedere l'universo attraverso gli occhi di un altro, di centinaia d'altri: di osservare il centinaio di universi che ciascuno di loro osserva, che ciascuno di loro e'. (Marcel Proust)}
\end{quotebox}

\begin{quotebox}\textit{Nessuna specie e' un'isola. (Mary Midgley)}
\end{quotebox}
...
\begin{quotebox}\textit{Non basta avere gli occhi per vedere (anonimo)}
\end{quotebox}
...
\begin{quotebox}\textit{Vivere non e' respirare: e' agire, e' fare uso degli organi, dei sensi, delle facolta', di tutte quelle parti di noi stessi per cui abbiamo il sentimento di esistere. (Jean-Jacques Rousseau)}
\end{quotebox}
...
\begin{quotebox}\textit{Chi non stima la vita, non la merita. (Leonardo da Vinci)}
\end{quotebox}
...
\begin{quotebox}\textit{Se il destino e' contro di noi, peggio per lui. (motto del 1º Reggimento Carabinieri Paracadutisti "Tuscania")}
\end{quotebox}
...
\label{tratti}
\begin{quotebox}\textit{
Chi dunque sa fare il bene e non lo compie, commette peccato. (Giacomo il Giusto 4.17, Lettera di Giacomo)}\end{quotebox}

\begin{quotebox}\textit{e' un diritto naturale saziarsi l'anima con la vendetta. (Attila)}\end{quotebox}

\begin{quotebox}\textit{Est Sularus Oth Mithas. (“Il mio onore e' la mia vita”, Giuramento dei Cavalieri di Solamnia.)}\end{quotebox}
...
\label{competenze}
\begin{quotebox}\textit{
``Chi dice che una cosa e' impossibile, non dovrebbe disturbare chi la sta facendo.''}\end{quotebox}
\begin{quotebox}\textit{``Non hai veramente capito qualcosa fino a quando non sei in grado di spiegarlo a tua nonna.''
\smallskip
Albert Einstein}\end{quotebox}
...
\begin{quotebox}\textit{C'e' solo un modo per allenarsi: quello giusto. (Carl Lewis)}\end{quotebox}
\begin{quotebox}\textit{
Wang Chi: Sei pronto?\\
...
\begin{quotebox}\textit{``Un mobile piu' lento non puo' essere raggiunto da uno piu' rapido; giacché quello che segue deve arrivare al punto che occupava quello che e' seguito e dove questo non e' piu' (quando il secondo arriva); in tal modo il primo conserva sempre un vantaggio sul secondo'' , Paradosso di Zenone}
\end{quotebox}
...
\label{vita-e-morte}
\begin{quotebox}\textit{Chi non conosce la morte, non conosce la vita. (Grand Hotel, film 1932)}
\end{quotebox}
...
\begin{quotebox}\textit{Applica sempre la giusta forza, mai troppa mai troppo poca. (Kano Jigoro)}
\end{quotebox}
...
\begin{quotebox}\textit{La forza non risiede in una Spada, ma nelle braccia di un valoroso. (The Legend of Zelda: Twilight Princess)}
\end{quotebox}
...
\begin{quotebox}\textit{Corre meno pericoli colui che, anche se e' al sicuro, sta in guardia. (Publilio Siro)}
\end{quotebox}
